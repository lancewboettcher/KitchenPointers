...
public class MakeSQL{
   ...
   public String makeNDBQuery(){
        String sql1 =
       "SELECT NDB_No, Shrt_Desc,MATCH (Shrt_Desc) AGAINST ('";
         String sql2 = 
       "' IN BOOLEAN MODE) AS score FROM nutritionFacts WHERE MATCH (Shrt_Desc) AGAINST ('";
         String sql3 = 
       "' IN BOOLEAN MODE) HAVING score > 0.5 order by score desc;";
   ...
   public String makeRecipeDBQuery(){
      String sql1 = 
      "SELECT * FROM recipeDB JOIN recipeIngredients on recipeDB.recipeID = recipeIngredients.recipeID WHERE ingredientID =";
      this.out = sql1 + this.in + ";";
   //   System.out.println(this.out);
      return this.out;
   }
   //Make the query to get the next number for the recipeID for the recipeDB databse.
   ...
   public String makeAddRecipe(int rID, int[] ingredient, String[] quantity, String name, String cuisine, String URL){
      ...
      for(int i = 0; i<ingredient.length; i++){
         ...
         + String.valueOf(ingredient[i]) + "','"
         + quantity[i] + "');\n";
      }
      String updateProtein = 
   "UPDATE recipeDB SET proteinCount= (SELECT SUM(P.Protein) FROM (SELECT t2.Protein FROM recipeIngredients as t1 Join nutritionFacts as t2 ON t1.ingredientID= t2.NDB_No WHERE t1.recipeID =" + String.valueOf(rID)+") as P) WHERE recipeID =" + String.valueOf(rID)+";\n";
      String updateCalorie = 
   "UPDATE recipeDB set calorieCount = (SELECT SUM(P.Energ_Kcal) FROM (SELECT t2.Energ_Kcal from recipeIngredients as t1 JOIN nutritionFacts as t2 on t1.ingredientID= t2.NDB_No WHERE t1.recipeID=" + String.valueOf(rID)+") as P) WHERE recipeID =" + String.valueOf(rID) + ";\n"; 
      String updateFat = 
 ...
 //     System.out.println(this.out);
   ...
   public static void main(String[] args){
     ...
     MakeSQL myString = new MakeSQL(foods);
     myString.makeNDBQuery();
     foods = "01152";
     myString = new MakeSQL(foods);
     myString.makeRecipeDBQuery();
